...
function generateToken(apiKey, expSeconds) {
    // Defensive check: Ensure apiKey is valid before splitting
    if (!apiKey || typeof apiKey !== 'string' || !apiKey.includes('.')) {
        throw new Error('Invalid Zhipu API Key format. It must contain a dot.');
    }
...
// ... (systemPrompts remain the same, no need to copy them here again) ...
const systemPrompts = {
...
exports.handler = async (event) => {
    ...
    try {
        ...
        const token = generateToken(zhipuApiKey, 3600);
        ...
        return {
            statusCode: 500,
            body: JSON.stringify({ error: '服务器发生未知错误 (Unknown server error)', details: error.message })
        };
    }
};